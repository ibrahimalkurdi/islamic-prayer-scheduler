...
| fetching the lists | there is no fetch button. Opening **either** version list runs `--list` and fills **both**, newest-first, sorted numerically so 1.0.10 sits above 1.0.9. A fetch under a minute old is reused, so opening the second list does not ask again. If the fetch fails, «تعذر جلب الإصدارات» is shown and the install list stays shut; the rollback list still opens when it holds the retained backup, which needs no network |
...
| **تحديث تلقائي يومي** | unticking writes `ENABLED=false`; ticking writes `ENABLED=true` **and** `PIN=`. Before ticking takes effect it runs `--latest`; if that names a version other than the installed one it asks «تفعيل التحديث التلقائي … هل تريد المتابعة؟», saying the device will be moved up to it or, when the installed version is newer, taken back to it. Yes enables and runs `--now` straight away; no leaves the box unticked. The box shows no tick while the question is open. If the pointer cannot be reached it enables without asking, and the 02:00 check moves the device when it can. Shown unticked while either `ENABLED=false` or a `PIN` is set, since both hold the device. Unticking stops the 02:00 check; **التحديث إلى أحدث إصدار** still works, because `ENABLED` is read only in cron mode |
| **يُفحص عن تحديث ويُثبَّت يوميًا الساعة 2:00 AM** | a grey line under the checkbox while it is ticked. The time is read from the `check_updates.sh` line in the live `crontab -l`, falling back to `config/crontab.txt`, so moving the cron line moves the label with no code change. Absent if neither has the line |
